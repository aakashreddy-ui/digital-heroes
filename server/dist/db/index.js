...
const dbPath = path_1.default.join(dataDir, 'digital_heroes.sqlite');
let rawDb = null;
...
async function initDatabase() {
  ...
  `);
    ...
    for (const plan of types_1.SUBSCRIPTION_PLANS) {
        const existing = exports.db.prepare('SELECT id FROM subscription_plans WHERE id = ?').get(plan.id);
        if (!existing) {
            exports.db.prepare(`
        INSERT INTO subscription_plans (id, name, price_cents, billing_interval, description, features, is_active, created_at)
        VALUES (?, ?, ?, ?, ?, ?, 1, ?)
      `).run(plan.id, plan.name, plan.price_cents, plan.billing_interval, plan.description, JSON.stringify(plan.features), new Date().toISOString());
        }
    }
    saveDbToDisk();
    return rawDb;
