...
async function seed() {
    ...
    const inactiveSubId = (0, uuid_1.v4)();
    const winnerUserId = (0, uuid_1.v4)();
    // Check if already seeded
    const existingAdmin = db_1.db.prepare('SELECT id FROM profiles WHERE email = ?').get('admin@digitalheroes.test');
    if (existingAdmin) {
        console.log('Database already contains seed data. Resetting demo data...');
        db_1.db.prepare('DELETE FROM winner_proofs').run();
        db_1.db.prepare('DELETE FROM payouts').run();
        db_1.db.prepare('DELETE FROM winners').run();
        db_1.db.prepare('DELETE FROM draw_entries').run();
        db_1.db.prepare('DELETE FROM prize_pools').run();
        db_1.db.prepare('DELETE FROM draws').run();
        db_1.db.prepare('DELETE FROM scores').run();
        db_1.db.prepare('DELETE FROM user_charities').run();
        db_1.db.prepare('DELETE FROM donations').run();
        db_1.db.prepare('DELETE FROM subscriptions').run();
        db_1.db.prepare('DELETE FROM profiles').run();
        db_1.db.prepare('DELETE FROM charities').run();
    }
    // Insert Profiles
    const insertProfile = db_1.db.prepare(`
    INSERT INTO profiles (id, email, password_hash, full_name, role, avatar_url, phone, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
    insertProfile.run(adminId, 'admin@digitalheroes.test', adminPasswordHash, 'Alexander Vance (Admin)', 'admin', 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80', '+1 555-0199', isoNow, isoNow);
    insertProfile.run(subscriberId, 'subscriber@digitalheroes.test', passwordHash, 'Marcus Reed (Active Hero)', 'subscriber', 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80', '+1 555-0142', isoNow, isoNow);
    insertProfile.run(inactiveSubId, 'inactive@digitalheroes.test', passwordHash, 'Claire Thornton (Inactive)', 'subscriber', 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150&auto=format&fit=crop&q=80', '+1 555-0177', isoNow, isoNow);
    insertProfile.run(winnerUserId, 'winner@digitalheroes.test', passwordHash, 'David Miller (Recent Winner)', 'subscriber', 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=150&auto=format&fit=crop&q=80', '+1 555-0188', isoNow, isoNow);
    // 2. Subscriptions
    const insertSub = db_1.db.prepare(`
  ...
  `);
    ...
    insertSub.run((0, uuid_1.v4)(), subscriberId, 'monthly', 'active', isoNow, nextMonth.toISOString(), isoNow, isoNow);
    insertSub.run((0, uuid_1.v4)(), winnerUserId, 'yearly', 'active', isoNow, nextYear.toISOString(), isoNow, isoNow);
    insertSub.run((0, uuid_1.v4)(), inactiveSubId, 'monthly', 'cancelled', lastMonth.toISOString(), isoNow, isoNow, isoNow);
    // Also give 15 other mock active subscribers for realistic prize pool calculations
    const mockSubIds = [];
    for (let i = 1; i <= 15; i++) {
        const mId = (0, uuid_1.v4)();
        mockSubIds.push(mId);
        insertProfile.run(mId, `hero${i}@digitalheroes.test`, passwordHash, `Hero Golfer #${i}`, 'subscriber', `https://api.dicebear.com/7.x/avataaars/svg?seed=Hero${i}`, `+1 555-01${i.toString().padStart(2, '0')}`, isoNow, isoNow);
        insertSub.run((0, uuid_1.v4)(), mId, i % 2 === 0 ? 'monthly' : 'yearly', 'active', isoNow, nextMonth.toISOString(), isoNow, isoNow);
    }
    ...
    const charities = [
        {
            ...
            upcoming_events: JSON.stringify([
                { id: (0, uuid_1.v4)(), title: 'Annual Junior Invitational', date: '2026-10-15', location: 'St. Andrews Youth Academy', description: 'Showcase tournament for 100 junior scholars.' },
                { id: (0, uuid_1.v4)(), title: 'STEM On The Green Clinic', date: '2026-11-04', location: 'Metropolitan Community Center', description: 'Teaching physics and geometry through ball flight dynamics.' }
            ]),
        ...
        },
        {
            ...
            is_featured: 1,
            upcoming_events: JSON.stringify([
                { id: (0, uuid_1.v4)(), title: 'Honor & Recovery Classic', date: '2026-10-28', location: 'Pine Valley Memorial Course', description: 'Fundraising scramble pairing veterans with civic leaders.' }
            ]),
            impact_metrics: JSON.stringify([
        ...
        },
        {
            ...
            is_featured: 0,
            upcoming_events: JSON.stringify([
                { id: (0, uuid_1.v4)(), title: 'Wetlands Planting Day', date: '2026-10-10', location: 'Evergreen Wildlife Sanctuary', description: 'Planting 2,500 native marsh grasses to filter natural runoff.' }
            ]),
            impact_metrics: JSON.stringify([
        ...
        },
        {
            ...
            is_featured: 1,
            upcoming_events: JSON.stringify([
                { id: (0, uuid_1.v4)(), title: 'Para-Champions Open', date: '2026-11-18', location: 'Sun Valley Accessible Resort', description: 'International tournament celebrating adaptive athletes.' }
            ]),
            impact_metrics: JSON.stringify([
        ...
        },
        {
            ...
            is_featured: 0,
            upcoming_events: JSON.stringify([
                { id: (0, uuid_1.v4)(), title: 'Clean Water Gala & Auction', date: '2026-12-05', location: 'Grand Horizon Pavilion', description: 'Fundraising dinner funding 15 community borehole wells.' }
            ]),
            impact_metrics: JSON.stringify([
    ...
    const insertCharity = db_1.db.prepare(`
  ...
  `);
    for (const c of charities) {
        insertCharity.run(c.id, c.name, c.slug, c.mission, c.description, c.category, c.logo_url, c.hero_image_url, c.website, c.is_featured, c.upcoming_events, c.impact_metrics, isoNow, isoNow);
    }
    // 4. User Charity Selections (Min 10%)
    const insertUserCharity = db_1.db.prepare(`
    INSERT INTO user_charities (id, user_id, charity_id, contribution_percentage, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?)
  `);
    insertUserCharity.run((0, uuid_1.v4)(), subscriberId, charities[0].id, 15, isoNow, isoNow); // 15% contribution
    insertUserCharity.run((0, uuid_1.v4)(), winnerUserId, charities[1].id, 20, isoNow, isoNow); // 20% contribution
    ...
    for (let i = 0; i < mockSubIds.length; i++) {
        ...
        insertUserCharity.run((0, uuid_1.v4)(), mockSubIds[i], charities[cIdx].id, pct, isoNow, isoNow);
    }
  ...
  `);
    ...
    for (const s of marcusScores) {
        insertScore.run((0, uuid_1.v4)(), subscriberId, s.score, s.date, s.course, s.notes, isoNow, isoNow);
    }
    ...
    for (const s of winnerScores) {
        insertScore.run((0, uuid_1.v4)(), winnerUserId, s.score, s.date, s.course, s.notes, isoNow, isoNow);
    }
    ...
    for (const subId of mockSubIds) {
        ...
        for (let day = 1; day <= 5; day++) {
            ...
            insertScore.run((0, uuid_1.v4)(), subId, cleanScore, `2026-09-${(10 + day).toString().padStart(2, '0')}`, 'Highland Hills Golf Club', 'Weekend medal round', isoNow, isoNow);
        }
    }
    // 6. Draws
    // A. Past Completed Draw: Draw #101 (August 2026)
    const pastDrawId = (0, uuid_1.v4)();
    const pastDrawNumbers = [33, 35, 37, 38, 42]; // David Miller matches 5 numbers!
    const insertDraw = db_1.db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
    insertDraw.run(pastDrawId, 101, '2026-08-31T20:00:00.000Z', '2026-08', 'completed', 'random', JSON.stringify(pastDrawNumbers), 50000, // $500 prior rollover
    220000, // $2,200 total pool
    '2026-08-31T20:05:00.000Z', adminId, '2026-08-01T00:00:00.000Z', isoNow);
  ...
  `).run((0, uuid_1.v4)(), pastDrawId, 220000, 170000, 50000, 0, 118000, // 40% of 170000 (68000) + 50000 rollover = 118000
    59500, // 35% of 170000
    42500, // 25% of 170000
    1, // 1 five-match winner (David Miller)
    2, // 2 four-match winners
    4, // 4 three-match winners
    118000, 29750, 10625, '2026-08-31T20:05:00.000Z');
    // Draw Entries for past draw
    const winnerEntryId = (0, uuid_1.v4)();
    db_1.db.prepare(`
  ...
  `).run(winnerEntryId, pastDrawId, winnerUserId, JSON.stringify([42, 37, 35, 33, 38]), JSON.stringify(pastDrawNumbers));
    // Marcus Reed entry in past draw (matched 3 numbers: 31, 36, 39 vs 33, 35, 37, 38, 42 -> 0 or 1 match)
    const marcusPastEntryId = (0, uuid_1.v4)();
    db_1.db.prepare(`
    INSERT INTO draw_entries (id, draw_id, user_id, numbers, matched_numbers, match_count, prize_tier, is_winner, created_at)
    VALUES (?, ?, ?, ?, ?, 1, NULL, 0, '2026-08-31T19:50:00.000Z')
  `).run(marcusPastEntryId, pastDrawId, subscriberId, JSON.stringify([38, 34, 36, 31, 39]), JSON.stringify([38]));
    // Winner record 1: David Miller (Paid)
    const paidWinnerId = (0, uuid_1.v4)();
    db_1.db.prepare(`
    INSERT INTO winners (
  ...
  `).run(paidWinnerId, pastDrawId, winnerUserId, winnerEntryId, 'https://images.unsplash.com/photo-1592919505780-303950717480?w=800&auto=format&fit=crop&q=80', '2026-09-01T10:00:00.000Z', adminId, '2026-09-01T14:30:00.000Z', 'Official handicap platform scorecard verified. Match confirmed.', '2026-09-02T09:00:00.000Z', 'PAY-20260902-88219', '2026-08-31T20:05:00.000Z', isoNow);
    // Winner record 2: Another winner pending verification for test
    const pendingWinnerEntryId = (0, uuid_1.v4)();
    db_1.db.prepare(`
    INSERT INTO draw_entries (id, draw_id, user_id, numbers, matched_numbers, match_count, prize_tier, is_winner, created_at)
    VALUES (?, ?, ?, ?, ?, 4, 'four_match', 1, '2026-08-31T19:50:00.000Z')
  `).run(pendingWinnerEntryId, pastDrawId, mockSubIds[0], JSON.stringify([33, 35, 37, 38, 20]), JSON.stringify([33, 35, 37, 38]));
    db_1.db.prepare(`
    INSERT INTO winners (
      id, draw_id, user_id, entry_id, prize_tier, prize_amount_cents,
  ...
  `).run((0, uuid_1.v4)(), pastDrawId, mockSubIds[0], pendingWinnerEntryId, 'https://images.unsplash.com/photo-1593111774240-d529f12cf4bb?w=800&auto=format&fit=crop&q=80', '2026-09-03T11:00:00.000Z', '2026-08-31T20:05:00.000Z', isoNow);
    // B. Current Active/Upcoming Draw: Draw #102 (September 2026)
    const upcomingDrawId = (0, uuid_1.v4)();
    insertDraw.run(upcomingDrawId, 102, '2026-09-30T20:00:00.000Z', '2026-09', 'draft', 'random', '[]', 75000, // $750 jackpot rollover
    245000, // $2,450 estimated pool
    null, null, isoNow, isoNow);
    // 7. Independent Donations
    const insertDonation = db_1.db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, 'succeeded', 1, ?)
  `);
    insertDonation.run((0, uuid_1.v4)(), subscriberId, charities[0].id, 5000, 'one_off', 'Marcus Reed', 'subscriber@digitalheroes.test', isoNow);
    insertDonation.run((0, uuid_1.v4)(), null, charities[1].id, 10000, 'monthly', 'Anonymous Donor', 'patron@impact.org', isoNow);
    insertDonation.run((0, uuid_1.v4)(), null, charities[3].id, 2500, 'one_off', 'Sarah Jenkins', 'sjenkins@gmail.com', isoNow);
