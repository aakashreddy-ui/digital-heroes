...
export const SUBSCRIBER_PRIZE_CONTRIBUTION_CENTS = 1000;
...
export const prizePoolService = {
   ...
   */
  calculatePrizePool(activeSubscriberCount: number, rolloverInCents: number = 0): {
    subscriberContributionCents: number;
    totalPoolCents: number;
    rolloverInCents: number;
  } {
    const safeCount = Math.max(0, activeSubscriberCount);
    const subscriberContributionCents = safeCount * SUBSCRIBER_PRIZE_CONTRIBUTION_CENTS;
   ...
   */
  calculateTierAmounts(subscriberContributionCents: number, rolloverInCents: number = 0): {
    tier5PoolCents: number;
    tier4PoolCents: number;
    tier3PoolCents: number;
  } {
    const tier5Base = Math.floor(subscriberContributionCents * TIER_5_PERCENTAGE);
    const tier4Pool = Math.floor(subscriberContributionCents * TIER_4_PERCENTAGE);
    const tier3Pool = Math.floor(subscriberContributionCents * TIER_3_PERCENTAGE);
   ...
   */
  calculateWinnerShare(tierPoolCents: number, winnerCount: number): number {
   ...
   */
  calculateJackpotRollover(tier5PoolCents: number, tier5WinnerCount: number): number {
